...
def getParser(program):
    ...
    if program == "AlphaImpute":
        ...
        core_impute_parser.add_argument(
            ...
            action="store_true",
            required=False,
            help="Flag to read in the files but not perform imputation.",
        )
        core_impute_parser.add_argument(
            "-no_phase",
            action="store_true",
            required=False,
            help="Flag to not do HD phasing initially.",
        )
        core_impute_parser.add_argument(
            "-maxthreads",
            default=1,
    ...
    if program == "AlphaPlantImpute":
        ...
        core_plant_parser.add_argument(
            "-plantinfo",
            default=None,
            required=False,
            type=str,
            nargs="*",
            help="A plant info file.",
        )

    if program == "AlphaMGS":
        core_assign_parser = parser.add_argument_group("Core assignement arguments")
        core_assign_parser.add_argument(
            "-potentialgrandsires",
            default=None,
            required=False,
            type=str,
            help="A list of potential dams for each individual.",
        )
        core_assign_parser.add_argument(
            "-usemaf",
            action="store_true",
            required=False,
            help="A flag to use the minor allele frequency when constructing genotype estimates for the sire and maternal grandsire. Not recomended for small input pedigrees.",
        )
    ...
    if program == "AlphaCall":
        ...
        call_parser.add_argument(
            ...
            required=False,
            type=float,
            help="Genotype calling threshold. Use. .3 for best guess genotype.",
        )
        call_parser.add_argument(
            "-sexchrom",
            action="store_true",
...
def readInPedigreeFromInputs(
    pedigree, args, genotypes=True, haps=False, reads=False, update_coding=False
):
    ...
    if bfile is not None:
        ...
        if alphaplinkpython_avail:
            for plink in args.bfile:
                if pedigreeReadIn is True:
                    print(
                        f"Pedigree file read in from -pedigree option. Reading in binary plink file {plink}. Pedigree information in the .fam file will be ignored."
                    )
                readInGenotypesPlink(pedigree, plink, startsnp, stopsnp, pedigreeReadIn)
        else:
            warnings.warn(
                "The module alphaplinkpython was not found. Plink files cannot be read in and will be ignored."
            )
    ...
    if ped is not None:
        for file in args.ped:
            pedigree.readInPed(
                file, startsnp, stopsnp, haps=False, update_coding=update_coding
            )
...
def writeOutGenotypesPlink(pedigree, fileName):
    ...
    if alphaplinkpython_avail:
        ...
        warnings.warn(
            "The module alphaplinkpython was not found. Plink files cannot be written out and will be ignored."
        )
