...
class Individual(object):
    def __init__(self, idx, idn, MetaFounder=None):
        ...
        self.inbred = False
        self.imputationAncestors = (
            []
        )  # This is a list of lists. Either length 0, length 1 or length 2.
        ...
        self.fileIndex = (
            dict()
        )  # Position of an animal in each file when reading in. Used to make sure Input and Output order are the same.
    ...
    def subset(self, start, stop):
        ...
        if self.genotypes is not None:
            new_ind.genotypes = self.genotypes[
                start:stop
            ].copy()  # Maybe could get away with not doing copies... doing them just to be safe.

        if self.haplotypes is not None:
            new_ind.haplotypes = (
                self.haplotypes[0][start:stop].copy(),
                self.haplotypes[1][start:stop].copy(),
            )

        if self.reads is not None:
            new_ind.reads = (
                self.reads[0][start:stop].copy(),
                self.reads[1][start:stop].copy(),
            )
        return new_ind
    ...
    def getGenotypedFounderStatus(self):
        ...
        if self.genotypedFounderStatus is None:
            if self.isFounder():
                ...
                parentStatus = max(
                    self.sire.getGenotypedFounderStatus(),
                    self.dam.getGenotypedFounderStatus(),
                )
                if parentStatus > 0:
...
class Pedigree(object):
    def __init__(self, fileName=None, constructor=Individual):
        ...
        self.referencePanel = (
            []
        )  # This should be an array of haplotypes. Or a dictionary?
    ...
    def writeOrder(self):
        if self.writeOrderList is None:
            inds = [
                ind
                for ind in self
                if (not ind.dummy) and (self.args.writekey in ind.fileIndex)
            ]
            self.writeOrderList = sorted_nicely(
                inds, key=lambda ind: ind.fileIndex[self.args.writekey]
            )

            if not self.args.onlykeyed:
                indsNoDummyNoFileIndex = [
                    ind
                    for ind in self
                    if (not ind.dummy) and (self.args.writekey not in ind.fileIndex)
                ]
                self.writeOrderList.extend(
                    sorted_nicely(indsNoDummyNoFileIndex, key=lambda ind: ind.idx)
                )

                dummys = [ind for ind in self if ind.dummy]
                self.writeOrderList.extend(
                    sorted_nicely(dummys, key=lambda ind: ind.idx)
                )
    ...
    def set_high_density(self):
        ...
        for individual in self:  # .individuals.values():
            is_high_density = (
                np.mean(individual.genotypes != 9) >= self.high_density_threshold
            )
            if is_high_density:
    ...
    def sort_individuals(self, individuals):
        return {
            k: v
            for k, v in sorted(
                individuals.items(), key=lambda pair: alphanum_key(pair[0])
            )
        }
        ...
        for ind in self:
            if not ind.isFounder():
                ...
                if parents in self.families:
                    ...
                    self.families[parents] = Family(
                        self.maxFam, ind.sire, ind.dam, [ind]
                    )
                    self.maxFam += 1
    ...
    def getIndividual(self, idx):
        if idx not in self.individuals:
            self.individuals[idx] = self.constructor(
                idx, self.maxIdn, MetaFounder=self.MainMetaFounder
            )
            self.maxIdn += 1
            self.generations = None
    ...
    def readInPedigreeFromList(self, pedList):
        ...
        for parts in pedList:
            ...
            self.maxIdn += 1
            self.individuals[idx].fileIndex["pedigree"] = index
            index += 1
        ...
        for parts in pedList:
            ...
            if sireID and not MFP:
                if sireID not in self.individuals:
                    self.individuals[sireID] = self.constructor(
                        sireID, self.maxIdn, MetaFounder=self.MainMetaFounder
                    )
            ...
            if damID and not MFM:
                if damID not in self.individuals:
                    self.individuals[damID] = self.constructor(
                        damID, self.maxIdn, MetaFounder=self.MainMetaFounder
                    )
    ...
    def readInFromPlink(self, idList, pedList, bed, externalPedigree=False):
        ...
        for i, idx in enumerate(idList):
            genotypes = bed[
                :, i
            ].copy()  # I think this is the right order. Doing the copy to be safe.
            nLoci = len(genotypes)
            if self.nLoci == 0:
                self.nLoci = nLoci
            if self.nLoci != nLoci:
            ...
            ind.fileIndex["plink"] = index
            index += 1

            if np.mean(genotypes == 9) < 0.1:
                ind.initHD = True
    ...
    def update_allele_coding(self, alleles):
        ...
        mask = alleles != b"0"
        mask &= (alleles != self.allele_coding[0]) & (
    ...
    def check_allele_coding(self, filename):
        ...
        # Unusual letters
        unusual = ~np.isin(
            self.allele_coding, [b"A", b"C", b"G", b"T", b"0"]
        )  # unexpected letters
        if np.sum(unusual) > 0:
    ...
    def readInPed(
        self, filename, startsnp=None, stopsnp=None, haps=False, update_coding=False
    ):
        ...
        data_list = MultiThreadIO.readLinesPlinkPlainTxt(
            filename, startsnp=startsnp, stopsnp=stopsnp, dtype=np.bytes_
        )
        index = 0
        ncol = None
        if self.nLoci != 0:
        ...
        for value in data_list:
            ind, alleles, ncol = self.check_line(
                value, filename, idxExpected=None, ncol=ncol, even_cols=True
            )
            ...
            if update_coding:
                ...
                if self.allele_coding is None:
                    self.allele_coding = np.full(
                        (2, self.nLoci // 2), b"0", dtype=np.bytes_
                    )
            ...
            if np.mean(ind.genotypes == 9) < 0.1:
                ind.initHD = True
    ...
    def readInGenotypes(self, fileName, startsnp=None, stopsnp=None):
        ...
        for value in data_list:
            ind, genotypes, ncol = self.check_line(
                value, fileName, idxExpected=None, ncol=ncol
            )

            ind.constructInfo(self.nLoci, genotypes=True)
            ind.genotypes = genotypes

            ind.fileIndex["genotypes"] = index
            index += 1
    ...
    def readInPhenotype(self, fileName):
        ...
        """
        data_list = MultiThreadIO.readLines(
            fileName, startsnp=None, stopsnp=None, dtype=np.float32
        )
    ...
    def readInPhenotypePenetrance(self, fileName):
        ...
        self.phenoPenetrance = (
            self.phenoPenetrance / np.sum(self.phenoPenetrance, 1)[:, None]
        )  # Normalising of the phenotype penetrance input so all rows sum to 1.
    ...
    def readInReferencePanel(self, fileName, startsnp=None, stopsnp=None):
        ...
        for value in data_list:
            ind, haplotype, ncol = self.check_line(
                value, fileName, idxExpected=None, ncol=ncol, getInd=False
            )
    ...
    def readInPhase(self, fileName, startsnp=None, stopsnp=None):
        ...
        for value in data_list:
            ...
            ind, haplotype, ncol = self.check_line(
                value, fileName, idxExpected=idxExpected, ncol=ncol
            )
            currentInd = ind
    ...
    def readInSequence(self, fileName, startsnp=None, stopsnp=None):
        ...
        data_list = MultiThreadIO.readLines(
            fileName, startsnp=startsnp, stopsnp=stopsnp, dtype=np.int64
        )
        e = 0
        currentInd = None
        ...
        for value in data_list:
            ...
            ind, reads, ncol = self.check_line(
                value, fileName, idxExpected=idxExpected, ncol=ncol
            )
            currentInd = ind

            ind.constructInfo(self.nLoci, reads=True)
            ind.fileIndex["sequence"] = index
            index += 1
    ...
    def readInAAP(self, fileName):
        ...
        if mfs_with_missing:
            missing_summary = "; ".join(
                [
                    f"{mfx} (loci: {', '.join(map(str, missing_values_per_mf[mfx]))})"
    ...
    def writeGenotypes_prefil(self, outputFile):
        ...
        print(
            "Output is using filled genotypes. Filling missing with rounded allele frequency"
        )
        self.setMaf()
